...
describe("Form", () => {
  ...
  it("calls onSave function when the name is defined", () => {
    ...
    const onSave = jest.fn();
    const {queryByText, getByAltText } = render(<Form interviewers={interviewers} onSave={onSave} student="Lydia Miller-Jones" /> );
  ...
  it("validates that an interviewer has been selected", () => {
    ...
    const onSave = jest.fn();
    const {getByText, getByAltText, queryByText} = render(<Form interviewers={interviewers} onSave={onSave} student="Lydia Miller-Jones"/> );
    fireEvent.click(getByText("Save"));

    expect(getByText(/interviewer not selected/i)).toBeInTheDocument();
    expect(onSave).not.toHaveBeenCalled();
    fireEvent.click(getByAltText("Sylvia Palmer"))
    fireEvent.click(getByText("Save"));
  
    expect(queryByText(/interviewer not selected/i)).toBeNull();
